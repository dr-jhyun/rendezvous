...
// you must do 'go get github.com/docker/docker' before 'go build'
...
var (
	...
	log          = goLog.New(os.Stdout).WithColor().WithDebug().WithoutTimestamp()
...
func readConfig() (*Config, error) {
	...
	case fileCmd.FullCommand():
		yamlData, err := io.ReadAll(*configFile)
...
func processPortProbe(address net.IP, port string, serverRunning bool) error {
	...
	// get response
	resposeAsBytes, _ := io.ReadAll(response.Body)
...
func portCheckHandler(response http.ResponseWriter, request *http.Request) {
	...
	_, _ = response.Write(responseAsBytes)
}
...
func dockerImagesHandler(response http.ResponseWriter, request *http.Request) {
	...
	response.Header().Set("Content-Type", "application/json")
	response.WriteHeader(http.StatusCreated)
	_, _ = response.Write(imagesAsBytes)
}
